...
//---------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////////////////////////////////
// БИБЛИОТЕКИ БИБЛИОТЕКИ БИБЛИОТЕКИ БИБЛИОТЕКИ БИБЛИОТЕКИ БИБЛИОТЕКИ БИБЛИОТЕКИ БИБЛИОТЕКИ БИБЛИОТЕКИ
///////////////////////////////////////////////////////////////////////////////////////////////////////////
// ГЛАВНОЕ МЕНЮ ГЛАВНОЕ МЕНЮ ГЛАВНОЕ МЕНЮ ГЛАВНОЕ МЕНЮ ГЛАВНОЕ МЕНЮ ГЛАВНОЕ МЕНЮ ГЛАВНОЕ МЕНЮ ГЛАВНОЕ МЕНЮ
///////////////////////////////////////////////////////////////////////////////////////////////////////////
int checker = 0;
...
//---------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////////////////////////////////
// СТУДЕНТЫ СТУДЕНТЫ СТУДЕНТЫ СТУДЕНТЫ СТУДЕНТЫ СТУДЕНТЫ СТУДЕНТЫ СТУДЕНТЫ СТУДЕНТЫ СТУДЕНТЫ СТУДЕНТЫ СТУДЕНТЫ
///////////////////////////////////////////////////////////////////////////////////////////////////////////
// СОТРУДНИКИ и УЧИТЕЛЯ СОТРУДНИКИ и УЧИТЕЛЯ СОТРУДНИКИ и УЧИТЕЛЯ СОТРУДНИКИ и УЧИТЕЛЯ СОТРУДНИКИ и УЧИТЕЛЯ
///////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void __fastcall TMainWindow::ComboBox10DropDown(TObject *Sender)
{
	...
	for (int i = 1; i < 5; i++) ComboBox10->Items->Add(""+IntToStr(i)+"-й курс");
	...
	while (!ADOQuery1->Eof)
	{
		if (ADOQuery1->FieldByName("IsEngaged")->AsInteger == 1)
		{
			...
			ComboBox10->Items->Strings[ADOQuery1->FieldByName("Course")->AsInteger - 1] = temp + " (Группе уже назначена пара)";
		}
		ADOQuery1->Next();
	}
...
void __fastcall TMainWindow::ComboBox13Select(TObject *Sender)
{
	...
	String TeachersName = ComboBox13->Text; String TeachersSubString = L" (Преподаватель занят)";
...
void __fastcall TMainWindow::Button3Click(TObject *Sender)
{
	if(ComboBox7->Text == ""  ||
	   ...
	   ComboBox12->Text == "" ||
	   ComboBox13->Text == "" ||
	   ComboBox14->Text == "") MessageDlg("Заполните все поля!", TMsgDlgType(mtInformation), TMsgDlgButtons() << mbOK, 0);
	else
	{
        String ClassroomName = ComboBox12->Text; String ClassroomSubString = L" (Аудитория занята)";
		String TeacherName = ComboBox13->Text; String TeacherSubString = L" (Преподаватель занят)";
...
void __fastcall TMainWindow::DateTimePicker1Change(TObject *Sender)
{
	...
	switch(dayOfWeek)
	{
		case 1: Label576->Caption = "Дата (день недели: воскресенье)"; break;
		case 2: Label576->Caption = "Дата (день недели: понедельник)"; break;
		case 3: Label576->Caption = "Дата (день недели: вторник)";     break;
		case 4: Label576->Caption = "Дата (день недели: среда)";       break;
		case 5: Label576->Caption = "Дата (день недели: четверг)";     break;
		case 6: Label576->Caption = "Дата (день недели: пятница)";     break;
		case 7: Label576->Caption = "Дата (день недели: суббота)";     break;
	}
}
//---------------------------------------------------------------------------
...
//	ADOQuery1->Parameters->ParamByName("Course")->Value = ComboBox18->ItemIndex + 1;
	...
	if(!ADOQuery1->IsEmpty())
	{
		...
		Label594->Caption = "Дата ("+ADOQuery1->FieldByName("Date")->AsString+")";
		TDateTime DateValue = VarToDateTime(ADOQuery1->FieldByName("Date")->AsString);
		DateTimePicker3->Date = DateValue;
		Label590->Caption = "Аудитория ("+ADOQuery1->FieldByName("ClassRoom")->AsString+")";
		Label591->Caption = "Преподаватель ("+ADOQuery1->FieldByName("Teacher")->AsString+")";
		Label592->Caption = "Дисциплина ("+ADOQuery1->FieldByName("Subject")->AsString+")";
		Label593->Caption = "Тип Занятия ("+ADOQuery1->FieldByName("TypeOfLesson")->AsString+")";
		Label595->Caption = "Пара ("+ADOQuery1->FieldByName("Pair")->AsString+"-я)";
		Label596->Caption = "Факультет ("+ADOQuery1->FieldByName("Faculty")->AsString+")";
		Label597->Caption = "Направление ("+ADOQuery1->FieldByName("Direction")->AsString+")";
		Label598->Caption = "Курс ("+ADOQuery1->FieldByName("Course")->AsString+"-й)";
	}
	else
	{
		Label590->Caption = "Аудитория";
		Label591->Caption = "Преподаватель";
		Label592->Caption = "Дисциплина";
		Label593->Caption = "Тип Занятия";
		Label595->Caption = "Пара";
		Label596->Caption = "Факультет";
		Label597->Caption = "Направление";
		Label598->Caption = "Курс";
...
void __fastcall TMainWindow::ComboBox26DropDown(TObject *Sender)
{
	...
	for (int i = 1; i < 5; i++) ComboBox26->Items->Add(""+IntToStr(i)+"-й курс");
	...
	while (!ADOQuery1->Eof)
	{
		if (ADOQuery1->FieldByName("IsEngaged")->AsInteger == 1)
		{
			...
			ComboBox26->Items->Strings[ADOQuery1->FieldByName("Course")->AsInteger - 1] = temp + " (Группе уже назначена пара)";
		}
		ADOQuery1->Next();
	}
}
...
void __fastcall TMainWindow::ComboBox21Select(TObject *Sender)
{
	...
	ADOQuery1->SQL->Clear();
	ADOQuery1->SQL->Add("SELECT S.Subject FROM Subjects S JOIN Teachers T ON T.Teacher = :Teacher JOIN TeacherSubjects TS ON S.SubjectId = TS.SubjectId WHERE T.TeacherId = TS.TeacherId");
	String TeachersName = ComboBox21->Text; String TeachersSubString = L" (Преподаватель занят)";
	int Tpos = TeachersName.Pos(TeachersSubString); if(Tpos > 0) TeachersName.Delete(Tpos, TeachersSubString.Length());
	ADOQuery1->Parameters->ParamByName("Teacher")->Value = TeachersName; ADOQuery1->Open();
...
void __fastcall TMainWindow::Button4Click(TObject *Sender)
{
	if(ComboBox17->Text == ""  ||
	   ...
	   ComboBox24->Text == "" ||
	   ComboBox25->Text == "" ||
	   ComboBox26->Text == "") MessageDlg("Заполните все поля!", TMsgDlgType(mtInformation), TMsgDlgButtons() << mbOK, 0);
	...
	else
	{
		...
		//ИЗБАВЛЕНИЕ ОТ (ЗАНЯТ)
		String ClassroomsName = ComboBox20->Text; String ClassroomsSubString = L" (Аудитория занята)";
		String TeachersName = ComboBox21->Text; String TeachersSubString = L" (Преподаватель занят)";
		int Tpos = TeachersName.Pos(TeachersSubString); if(Tpos > 0) TeachersName.Delete(Tpos, TeachersSubString.Length());
...
// СОТРУДНИКИ и УЧИТЕЛЯ СОТРУДНИКИ и УЧИТЕЛЯ СОТРУДНИКИ и УЧИТЕЛЯ СОТРУДНИКИ и УЧИТЕЛЯ СОТРУДНИКИ и УЧИТЕЛЯ
///////////////////////////////////////////////////////////////////////////////////////////////////////////
// АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ АДМИНЫ
///////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void __fastcall TMainWindow::LabeledEdit1Change(TObject *Sender)
{
	...
	{
//		MessageDlg("Такой студент уже имеет карточку студента. Если вы хотите изменить данные о студенте, то перейдите во вкладку 'Изменение карточек' -> 'Cтуденты'", TMsgDlgType(mtInformation), TMsgDlgButtons() << mbOK, 0);
...
void __fastcall TMainWindow::LabeledEdit17Change(TObject *Sender)
{
	...
	if(ADOQuery1->RecordCount != 0)
	{
//		MessageDlg("Такой преподаватель уже имеет карточку. Если вы хотите изменить данные о нём, то перейдите во вкладку 'Изменение карточек' -> 'Преподаватели'", TMsgDlgType(mtInformation), TMsgDlgButtons() << mbOK, 0);
...
void __fastcall TMainWindow::LabeledEdit36Change(TObject *Sender)
{
	...
	if(ADOQuery1->RecordCount != 0)
	{
//        MessageDlg("Такой сотрудник уже имеет карточку. Если вы хотите изменить данные о нём, то перейдите во вкладку 'Изменение карточек' -> 'Сотрудники'", TMsgDlgType(mtInformation), TMsgDlgButtons() << mbOK, 0);
