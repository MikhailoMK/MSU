...
{
	Clipboard()->AsText = THashMD5::GetHashString(Edit1->Text);
//	ShowMessage(THashMD5::GetHashString(Edit1->Text));
	if(Edit1->Text != "")
	{
		...
		{
			MessageDlg("Неправильный пароль!", TMsgDlgType(mtInformation), TMsgDlgButtons() << mbOK, 0);
		}
	}
	else
	{
		MessageDlg("Некорректный пароль!", TMsgDlgType(mtInformation), TMsgDlgButtons() << mbOK, 0);
	}
}
//---------------------------------------------------------------------------
