...
void __fastcall TLoginWindow::Button1Click(TObject *Sender)
{
	if(Edit1->Text != "")
	{
		...
		{
			MessageDlg("Пользователь не найден!", TMsgDlgType(mtInformation), TMsgDlgButtons() << mbOK, 0);
		}
	}
	else
	{
		MessageDlg("Некорректный логин!", TMsgDlgType(mtInformation), TMsgDlgButtons() << mbOK, 0);
	}
}
//---------------------------------------------------------------------------
