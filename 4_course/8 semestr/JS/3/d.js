...
function drawRightTriangle(base, hypotenuse) {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.strokeStyle = 'black';
    ctx.fillStyle = 'white';
    ctx.beginPath();
    const height = Math.sqrt(hypotenuse ** 2 - base ** 2);
    const x = (canvas.width - base) / 2;
    const y = (canvas.height + height) / 2;
    ctx.moveTo(x, y);
    ctx.lineTo(x + base, y);
...
function highlightRightTriangle(property) {
    ...
    const height = Math.sqrt(hypotenuse ** 2 - base ** 2);
    drawRightTriangle(base, hypotenuse);
    ctx.strokeStyle = 'red';
    ctx.lineWidth = 3;
    ctx.shadowBlur = 10;
    ctx.shadowColor = 'rgba(255, 0, 0, 0.5)';
...
function highlightEllipse(property) {
    ...
    if (property === 'majorAxis') {
        ...
        ctx.strokeRect(canvas.width / 2 - majorAxis / 2, canvas.height / 2 - minorAxis / 2, majorAxis, minorAxis);
    } else if (property === 'area') {
        ctx.fillStyle = 'rgba(255, 0, 0, 0.3)';
        ctx.beginPath();
        ctx.ellipse(canvas.width / 2, canvas.height / 2, majorAxis / 2, minorAxis / 2, 0, 0, 2 * Math.PI);
...
function clearHighlight() {
    ...
    switch (shape) {
        ...
        case 'rightTriangle':
            drawRightTriangle(
                parseFloat(document.getElementById('base').value || 0),
                parseFloat(document.getElementById('hypotenuse').value || 0)
            );
            break;
