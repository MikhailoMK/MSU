...
var
  ...
  function RotateMatrix(a: ddm; n: integer; m:integer; clockwise: boolean): ddm; export;stdcall;
...
implementation
  ...
  function RotateMatrix;external 'C:\Users\akhli\OneDrive\Рабочий стол\matrix\dll\Win32\Debug\matrix.dll' name 'RotateMatrix';
...
var Matrix2:ddm;
var start :integer:=(Radiogroup3.ItemIndex);
var i,j:integer;
var res:RealArray;
//      finish := strtoint(edit6.Text);
//      Matrix := rotateMatrix(Matrix, size, size, true);
Setlength(Matrix2,n1,n1);
SetLength(matrix, n1, m1);
matrix:=fillMD(n1,m1);

      if start = 1 then  start := 3
      else if start = 2 then start := 1
      else if start =3  then  start := 2;

      for i := 0 to start-1 do matrix := rotateMatrix(matrix, n1,m1, true);
      if RadioGroup5.ItemIndex = 1 then
       begin
       for i := 0 to StringGrid1.ColCount - 1 do
        for j := 0 to StringGrid1.RowCount - 1 do
          matrix2[i,j] := matrix[j,i];
          res:=SpiralMatrixToArray(matrix2,n1,m1,true);
       end
       else
         res:=SpiralMatrixToArray(matrix,n1,m1,true);

       var size:= length(res);
       StringGrid4.ColCount:=size;
       for i := 0 to size - 1 do
        begin
           stringgrid4.Cells[i, 0] := floattostr(res[i]);
        end;
end;
...
var res:RealArray;
setlength(res, m1*n1);
for j := 0 to n1*m1-1 do
  res[j] := strtofloat(stringgrid4.Cells[j,0]);
//var size := Trunc(Sqrt(length(res)));
matrix2 := ArrayToSpiralMatrix(res,m1,m1);
setlength(Matrix,n1,n1);

if start = 3  then start := 2
else if start = 2 then  start := 3;



for i := 0 to start-1 do Matrix2 := rotateMatrix(Matrix2, n1, n1, true);
...
begin
  ...
  begin
    for i := 0 to n1 - 1 do
      for j := 0 to n1 - 1 do
        matrix[n1-1-i,n1-1-j] := Matrix2[j,i]
...
//      Matrix2 := Matrix;
      if RadioGroup5.ItemIndex = 1 then
       ...
       for i := 0 to n1 - 1 do
        for j := 0 to n1- 1 do
          // Присвойте значение каждой ячейки в матрицу
          MAtrix2[i,j] := Matrix[j,i];
          res:=ReadMatrixByCorners(MAtrix2,n1,m1);
       end
       else
         res:=ReadMatrixByCorners(MAtrix,n1,m1);
...
for i := 0 to n1*m1-1 do
  begin
    StringGrid4.Cells[i,0]:=FloatToStr(Random(100));
  end;
end;
...
begin
  ...
  StringGrid4.ColCount:=StrToint(Edit1.Text)*StrToInt(Edit2.Text);
...
for j := 0 to n1*n1-1 do
  res[j] := strtofloat(stringgrid4.Cells[j,0]);
matrix2 := WriteMatrixByCorners(res,n1,n1);
setlength(Matrix,m1,m1);
if start = 3  then start := 2
else if start = 2 then  start := 3;
for i := 0 to start-1 do Matrix2 := rotateMatrix(Matrix2, n1, n1, true);
 if RadioGroup5.ItemIndex = 1 then
...
showMD(matrix,n1,m1);
 ...
 matrix:=WriteMatrixByCorners(a,n1,m1);
 showMD(matrix,n1,m1);  }
