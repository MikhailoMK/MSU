...
begin
  try
    ...
    WordDoc := WordApp.Documents.Add(path + 'Сертификат.dotx');
    WordDoc.Activate;
    for i := 1 to 3 do
    begin
     WordApp.Selection.Find.Execute('##' + IntToStr(i) + '##', True, True, False, False, False, True, 1, False, textBoxValues[i], 2);
    end;
    WordDoc.SaveAs('F:\3 курс\Офисные технологии\1\Результат\' + textBoxValues[1] + '.docx');
    WordDoc.Close;
    WordApp.Quit;
    ShowMessage('Сертификат готов!');
  except
